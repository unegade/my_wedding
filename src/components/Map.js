...
const Map = () => {
    const styles = { height: "50vh", filter: "hue-rotate(160deg) grayscale(30%)", borderRadius: "16px" }
    
    return <div>
        <img src={locationIcon} width={50} alt=''/>
        <h2>Местоположение</h2>
        <div>г.Екатеринбург, ул.Восточная, д.51</div>
        <div>Заезд через шлакбаум, при въезде нажать кнопку. Парковка для вас будет бесплатной.</div>
        <div>Вход в здание через железную дверь, подняться на лифте  на 8й этаж, затем по лестнице на 9й этаж.</div>
